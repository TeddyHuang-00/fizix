...
impl Mul<Vector> for f64 {
    ...
    fn mul(self, rhs: Vector) -> Self::Output {
        Vector([rhs.0[0] * self, rhs.0[1] * self, rhs.0[2] * self])
    }
}
...
fn main() {
    ...
    let force_a = newton(Vector::new(1.0, 0.0, 0.0));
    let force_b = newton(Vector::new(0.0, 2.0, 0.0));
    let force_c = newton(Vector::new(0.0, 0.0, 2.0));
    ...
    let velocity = meter(Vector::new(0.0, 1.0, 0.0)) / second(1.0);
    // And create electric and magnetic fields
    let electric_field = volt(Vector::new(0.0, 1.0, 0.0)) / meter(1.0);
    let magnetic_field = tesla(Vector::new(0.0, 0.0, 10.0));
